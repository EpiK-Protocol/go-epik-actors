...
func CheckStateInvariants(st *State, store adt.Store, priorEpoch abi.ChainEpoch, balance abi.TokenAmount) (*StateSummary, *builtin.MessageAccumulator, error) {
	...
	acc.Require(big.Add(totalReward, balance).Equals(StorageMiningAllocationCheck), "reward given %v + reward left %v != storage mining allocation %v", totalReward, balance, StorageMiningAllocationCheck)
