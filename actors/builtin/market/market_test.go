...
func TestPublishStorageDeals(t *testing.T) {
	...
	t.Run("publish a deal after activating a previous deal which has a start epoch far in the future", func(t *testing.T) {
		...
		rt.SetEpoch(publishEpoch)
		deal1ID := actor.generateAndPublishRandDeal(rt, client, mAddr, startEpoch /* endEpoch, */, startEpoch)
		...
		newEpoch := publishEpoch + 1
		rt.SetEpoch(newEpoch)
		deal2ID := actor.generateAndPublishRandDeal(rt, client, mAddr, startEpoch+1 /* endEpoch+1, */, startEpoch+1)
...
func TestOnMinerSectorsTerminate(t *testing.T) {
	...
	t.Run("terminate multiple deals from multiple providers", func(t *testing.T) {
		...
		dealId4 := actor.generateAndPublishRandDealWithLabel(rt, client, maddrs2, startEpoch /* endEpoch, */, startEpoch, "deal4")
		dealId5 := actor.generateAndPublishRandDealWithLabel(rt, client, maddrs2, startEpoch /* endEpoch+1, */, startEpoch, "deal5")
