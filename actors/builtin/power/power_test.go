...
func TestConstruction(t *testing.T) {
	...
	t.Run("create miner", func(t *testing.T) {
		...
		actor.createMiner(rt, owner, owner, coinbase, miner, actr, abi.PeerID("miner"), []abi.Multiaddrs{{1}}, abi.RegisteredSealProof_StackedDrg8MiBV1_1, abi.NewTokenAmount(10))
		...
		assert.True(t, found)
		assert.Equal(t, power.Claim{abi.RegisteredSealProof_StackedDrg8MiBV1_1, big.Zero(), big.Zero(), abi.NewTokenAmount(0)}, actualClaim) // miner has not proven anything
...
func TestCreateMinerFailures(t *testing.T) {
	...
	t.Run("fails if send to Init Actor fails", func(t *testing.T) {
		...
		msgParams := &initact.ExecParams{
			...
			ConstructorParams: initCreateMinerBytes(t, owner, owner, owner, peer, mAddr, sealProofType),
		}
...
func TestPowerAndPledgeAccounting(t *testing.T) {
	...
	powerUnit, err := builtin.ConsensusMinerMinPower(abi.RegisteredSealProof_StackedDrg8MiBV1_1)
	require.NoError(t, err)
	...
	t.Run("new miner updates MinerAboveMinPowerCount", func(t *testing.T) {
		for _, test := range []struct {
			...
			proof:          abi.RegisteredSealProof_StackedDrg2KiBV1_1, // 2K sectors have zero consensus minimum
			expectedMiners: 1,
		}, {
			version:        network.Version6,
			proof:          abi.RegisteredSealProof_StackedDrg8MiBV1_1,
			expectedMiners: 0,
		}, {
...
func TestCron(t *testing.T) {
	...
	t.Run("handles failed call", func(t *testing.T) {
		...
		rawPow, err := builtin.ConsensusMinerMinPower(abi.RegisteredSealProof_StackedDrg8MiBV1_1)
		require.NoError(t, err)
...
func (h *spActorHarness) createMiner(rt *mock.Runtime, owner, worker, coinbase, miner, robust addr.Address, peer abi.PeerID,
	...
	msgParams := &initact.ExecParams{
		CodeCID:           builtin.StorageMinerActorCodeID,
		ConstructorParams: initCreateMinerBytes(h.t, owner, worker, coinbase, peer, multiaddrs, sealProofType),
	}
...
func (h *spActorHarness) createMinerBasic(rt *mock.Runtime, owner, worker, miner addr.Address) {
	...
	h.minerSeq += 1
	h.createMiner(rt, owner, worker, owner, miner, actrAddr, abi.PeerID(label), nil, h.sealProof, big.Zero())
}
...
func initCreateMinerBytes(t testing.TB, owner, worker, coinbase addr.Address, peer abi.PeerID, multiaddrs []abi.Multiaddrs, sealProofType abi.RegisteredSealProof) []byte {
